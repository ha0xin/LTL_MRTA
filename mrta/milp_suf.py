...
def construct_milp_constraint(
    ...
    ) = create_variables(m, ts, poset, pruned_subgraph, element2edge, type_num, minimal_element, maximal_element)
    ...
    for element in poset:
        ...
        if edge_label != "1" and element in minimal_element:  # extra literals with other clauses in the edge
        ...
        elif edge_label == "1" and element in minimal_element:  # extra literals
            extra_node = list(final_element_type_robot_node[element].values())[0]
            m.addConstr(
                quicksum(
                    [
            ...
            for c in clause:
                for l in c:
                    for vertex in range(num_vertex):
                        ...
                        m.addConstr(
                            quicksum(x_vars[(p, w, id2robots[robot][vertex])] for p in ts.predecessors(w))
                            == c_vars[l[:-1]]
                        )
    ...
    # one and only one clause for extra literals is true -- (30)
    m.addConstr(quicksum(b_minimal_element_vars[element] for element in minimal_element) == 1)
...
def create_variables(m, ts, poset, pruned_subgraph, element2edge, type_num, minimal_element, maximal_element):
...
def one_clause_true(
    ...
    if component == 1 and element in minimal_element:
        m.addConstr(
            quicksum([c_vars[(element, component, c)] for c in range(len(label))] + [b_minimal_element_vars[element]])
            == 1
        )
...
def edge_constraints(
    ...
    if element in minimal_element:
        ...
        for i in clause_nodes:
            m.addConstr(
                quicksum(
                    t_vars[(clause_nodes[0], k, 1)]
                    for k in range(type_num[ts.nodes[clause_nodes[0]]["location_type_component_element"][1]])
                )
                == quicksum(
                    t_vars[(i, k, 1)] for k in range(type_num[ts.nodes[i]["location_type_component_element"][1]])
                )
            )

        # one of the extra literals in the last subtasks must be satisfied -- (31)
        ...
        m.addConstr(
            ...
            * (
                quicksum(
                    ...
                    for another_element in minimal_element
        ...
        # if the extra literal is one, all robots return to initial locations -- (32)
        expr_literal = quicksum(
            x_vars[(p, i, type_robot[1])]
            for type_robot, i in final_element_type_robot_node[element].items()
            for p in ts.predecessors(i)
        )
    ...
    # the edge label becomes true at 0, following after (14)
