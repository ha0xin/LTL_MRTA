...
class Workspace(object):
    ...
    def point_to_point_path(self):
        ...
        for l1 in range(len(self.regions)):
            for l2 in range(l1, len(self.regions)):
                length, path = nx.algorithms.single_source_dijkstra(
                    self.graph_workspace, source=self.regions[key_region[l1]], target=self.regions[key_region[l2]]
                )
        ...
        for r1 in range(len(self.label_location)):
            for l1 in range(len(self.regions)):
                length, path = nx.algorithms.single_source_dijkstra(
                    self.graph_workspace, source=self.label_location[key_init[r1]], target=self.regions[key_region[l1]]
                )
