...
print_red_on_cyan(type_robot_location)
...
for pair, _ in init_acpt:
    # reset the workspace for each pair of initial and accepting states
    ...
    for _, poset_relation, pos, hasse_diagram in hasse_graphs:
        ...
        if buchi.ap_sat_label(
            ...
            if best_cost >= cost:
                ...
                best_cost = cost
                print(
                    f"{seqsim} + col:{collision[seqsim]}: cost of the best pre + suf (loop) parts {best_cost}, horizon {len(best_path[(1, 0)])}"
                )
            if show:
        ...
        for _, poset_relation_suf, pos_suf, hasse_diagram_suf in hasse_graphs_suf:
            ...
            # --------------------- MILP -------------------------
            maximal_element_suf = [node for node in hasse_diagram_suf.nodes() if hasse_diagram_suf.in_degree(node) == 0]
            ...
            if best_cost >= cost:
                ...
                best_cost = cost
                print(
                    f"{seqsim} + col:{collision[seqsim]} cost of the best pre + suf parts {best_cost}, horizon {len(robot_path_pre[(1, 0)]) + len(robot_path_suf[(1, 0)])}"
                )
